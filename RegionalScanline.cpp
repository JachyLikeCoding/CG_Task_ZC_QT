...
bool RegionalScanlineProcessor::cmp(const ActivePolygon &a, const ActivePolygon &b)
{
	return a.maxZ > b.maxZ;
}

void RegionalScanlineProcessor::initProcessor(Object& _obj) {
	obj = _obj;
...
void RegionalScanlineProcessor::buildCPT() {
	...
	for (int i = 0; i < obj.faces.size(); i++) {
		...
		ClassifiedPolygon newCP(maxZ, i, a, b, c, d, dy, color);
		qDebug() << "maxY:" << maxY << endl;
		std::cout << "i=" << i << endl;
		CPT[maxY].push_back(newCP);
	}
}
void RegionalScanlineProcessor::buildCET() {
...
bool RegionalScanlineProcessor::addPolygonToAPT(int y, vector<ActivePolygon>& APT,
	...
	for (int i = 0; i < CPT[y].size(); i++) {
		...
		ActivePolygon ap(cp.maxZ, cp.polygon_id, cp.a, cp.b, cp.c, cp.d, cp.dy, cp.color);
		APT.push_back(ap);
	}
	return true;
}
bool RegionalScanlineProcessor::addEdgeToAET(vector<ClassifiedEdge>& CET_y, vector<ActiveEdge>& AET,
...
void RegionalScanlineProcessor::RegionalScanline(Object& _obj) {
	initProcessor(_obj);
	buildCPT();
	buildCET();
...
bool RegionalScanlineProcessor::update_APT(ActivePolygon& AP) {
	AP.remain_dy -= 1;
	if (AP.remain_dy < 0) {
		return false;
	}
...
bool RegionalScanlineProcessor::update_AET(ActiveEdge& AE, vector<ClassifiedEdge>& CET_y) {
	AE.dyl -= 1;
	AE.dyr -= 1;
...
int RegionalScanlineProcessor::findNextCE(ActiveEdge& AE, GLfloat x, vector<ClassifiedEdge>& CET_y) {
...
bool RegionalScanlineProcessor::update_APTAET(vector<ActivePolygon>& APT, vector<ActiveEdge>& AET,
...
void RegionalScanlineProcessor::resizeWindow(int width, int height) {
	winHeight = height;
	winWidth = width;
	CPT.clear();
	CET.clear();
...
//TODO:add regional scanline !
bool RegionalScanlineProcessor::updateBuffer(vector<ActiveEdge>& AET, int y) {
	...
	for (auto iter = AET.begin(); iter != AET.end(); ++iter) {
		for (int x = int(iter->xl); x <int(iter->xr); x++) {
...
vec3 RegionalScanlineProcessor::getColor(int polygon_id, vector<ActivePolygon>& APT) {
	vec3 color;
	for (int i = 0; i < APT.size(); i++) {
...
void RegionalScanlineProcessor::clearDS() {
	AET.clear();
	APT.clear();
	CET.clear();
	CPT.clear();
...
// just for debug:
void RegionalScanlineProcessor::test() {
	qDebug() << "CET size:" << CET.size() << endl;
	qDebug() << "CPT size:" << CPT.size() << endl;
}
void RegionalScanlineProcessor::printAET(vector<ActiveEdge>& AET) {
...
void RegionalScanlineProcessor::printAPT(vector<ActivePolygon>& APT) {
